...
// Modified by Brian House to support float values for frequency and duty cycle, and duty cycle in percentage
...
void toneAC(float frequency, float dutycycle, unsigned long length, uint8_t background) {
  if (frequency == 0) { noToneAC(); return; } // If frequency or volume are 0, turn off sound and return.
  ...
  // unsigned int duty = top / _tAC_volume[volume - 1]; // Calculate the duty cycle (volume).
    unsigned int duty = top / (100.0 / dutycycle); // Calculate the duty cycle (volume).
